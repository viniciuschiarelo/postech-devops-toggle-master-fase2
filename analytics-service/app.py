...
# Endpoints opcionais para desenvolvimento local (DynamoDB Local / ElasticMQ)
DYNAMODB_ENDPOINT = os.getenv("DYNAMODB_ENDPOINT_URL")  # ex: http://dynamodb-local:8000
SQS_ENDPOINT = os.getenv("SQS_ENDPOINT_URL")            # ex: http://sqs-local:9324
...
try:
    ...
    sqs_kwargs = {"endpoint_url": SQS_ENDPOINT} if SQS_ENDPOINT else {}
    dynamodb_kwargs = {"endpoint_url": DYNAMODB_ENDPOINT} if DYNAMODB_ENDPOINT else {}
    ...
    if SQS_ENDPOINT:
        log.info(f"SQS usando endpoint local: {SQS_ENDPOINT}")
    if DYNAMODB_ENDPOINT:
        log.info(f"DynamoDB usando endpoint local: {DYNAMODB_ENDPOINT}")
